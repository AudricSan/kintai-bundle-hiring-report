# Contributing to kintai-bundle-hiring-report
...
- Namespace root: `kintai\Bundles\Installed\HiringReport\` → `src/`
...
- Persistence goes through `HiringReportRepositoryInterface` — but unlike
  most bundles, this one is bound by Kintai Core, not by this repo (see
  [CLAUDE.md](CLAUDE.md#architecture) for why). Never add a binding for it
  here; Core's `AdminUserController` depends on it staying Core-owned.
